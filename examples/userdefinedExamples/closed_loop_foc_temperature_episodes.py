...
def main():
    ...
    # =========================================================================
    # 3. CONTROLLER TUNING (Tuned at nominal 20 deg C)
    # =========================================================================
    fc_spd = 10
    ...
    # =========================================================================
    # 4. ENVIRONMENT & CONTROLLER CREATION
    # =========================================================================
    ...
    controller = Controller.make(
        ...
        decoupling=False,
        plot_torque=False,  # Headless execution
    )
    ...
    print(f"================================================================")
    print(f"Running Multi-Episode Temperature Sensitivity Simulation ({len(temperatures)} Episodes)")
    print(f"================================================================")
    ...
    for ep_idx, T_celsius in enumerate(temperatures):
        ...
        # Dynamically inject temperature parameters into the physical plant model
        ...
        print(f"  -> PM Flux (psi_p): {psi_p_T:.4f} Vs ({delta_psi_pct:+.1f}%)")
        print(f"  -> Stator Resistance (Rs): {Rs_T:.3f} Ohm ({delta_rs_pct:+.1f}%)")
    ...
    fig.suptitle("FOC Temperature Sensitivity Analysis Across Multiple Episodes", fontsize=13, fontweight="bold")

    # 1. Speed Tracking
    for idx, ep in enumerate(episode_results):
        lbl = f"T = {ep['temp']:.0f}°C (ψ_p = {ep['psi_p']:.3f} Vs, Rs = {ep['r_s']:.2f} Ω)"
    ...
    axes[0].set_title("1. Speed Response vs. Operating Temperature", fontsize=11)
    axes[0].grid(True, linestyle=":", alpha=0.7)
    axes[0].legend(loc="lower right", fontsize=9)

    # 2. Quadrature Current i_sq (Compensating for flux reduction)
    for idx, ep in enumerate(episode_results):
    ...
    axes[1].set_title("2. Quadrature Current (i_sq) Demand (Higher i_sq needed at higher Temp to produce same torque)", fontsize=11)
    ...
    axes[2].set_title("3. Direct Current (i_sd) Response", fontsize=11)
    axes[2].grid(True, linestyle=":", alpha=0.7)
    axes[2].legend(loc="upper right", fontsize=9)

    plt.tight_layout()
    png_path = os.path.join(current_dir, "temperature_episodes_comparison.png")
    ...
    plotly_fig = make_subplots(
        ...
        subplot_titles=[
            "1. Rotor Speed Response Across Temperatures [RPM]",
            "2. Quadrature Current Demand (i_sq) [A]",
            "3. Direct Current (i_sd) [A]",
        ],
    )
    ...
    for idx, ep in enumerate(episode_results):
        t_label = f"{ep['temp']:.0f}°C (ψ={ep['psi_p']:.3f}Vs, R={ep['r_s']:.2f}Ω)"
        c = colors[idx]
        # Speed
        plotly_fig.add_trace(
    ...
    plotly_fig.update_layout(
        title=dict(
            text="<b>Multi-Episode Temperature Sensitivity Dashboard (ψ_p and R_s vs. Temp)</b>",
            x=0.5,
            xanchor="center",
            font=dict(size=18),
        ),
    ...
    html_path = os.path.join(current_dir, "temperature_episodes_dashboard.html")
